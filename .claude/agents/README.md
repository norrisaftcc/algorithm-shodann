...
| `linx-voice-readability-editor` | Voice and readability gate for SHODANN prose. Audits rendered reviews and other student-facing text for the SHODANN voice, forbidden vocabulary, 400-word cap, and clearance-appropriate register. | A rendered SHODANN review from `python scripts/dev.py render` and the prompt text it would be asked to polish. Flagged the voice mismatch and missing substitutions that a mechanical validator would not catch. |
| `shodann` | The Algorithm's own voice as a dispatchable seat. Reviews a change by its *movement* against the prior state of the same work, never its absolute position, and matches the clearance band's posture - teaches to YELLOW, mentors at GREEN, reports at BLUE+. Holds no Write tool, so "nothing is written" is enforced by the tool list rather than by wording. | Shared base, not yet exercised here. Byte-identical to the copy in `the_intern`; both are expected to diverge as each repository adjusts its own. |
| `shodann-voice` (skill) | The register without the seat: modes, the ladder, the RAGE STATE vocabulary shifts, the 400-word cap, the never-says list. For writing or auditing citizen-facing prose by hand. | Same shared base. `linx-voice-readability-editor` is the editor that applies rules to rendered prose; this is the statement of them. |
...
`kevin-repo-warden`, `program-advisor` and `spine-owner` have SHODANN analogues worth retargeting, in that order. `cadence-master`, `module-builder` and `liza-theme-skinner` have none — leave them, and delete them if they are still unused once the fleet has been exercised.
